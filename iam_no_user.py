'''
Rule Name:
  IAM_USER_NOT_ALLOWED_EXCEPT

Description:
  To ensure there is no IAM users except if explicitly allowed.

Rule Parameters:
  | ------------------- | --------- | ----------------------------------------------- |
  | Parameter Name      | Type      | Description                                     |
  | ------------------- | --------- | ----------------------------------------------- |
  | WhitelistedUserList | Optional  | List pf allowed IAM users, seperated by commas. |
  | ------------------- | --------- | ----------------------------------------------- |
...
Feature:
  ...
  I want: To ensure IAM users are not present unless from an allowed List.
...
Scenarios:
  ...
  Scenario: 3
    Given: WhitelistedUserList is not empty
      And: each User ID is an alphanumerical string starting with "AIDA"
      And: All IAM Users UniqueID is listed in WhitelistedUserList
     Then: Return Compliant

  Scenario: 4
    Given: WhitelistedUserList is not empty
      And: WhitelistedUserList is configured correctly
      And: At least one IAM User UniqueID is not listed in WhitelistedUserList
...
def evaluate_parameters(rule_parameters):
    ...
    if "WhitelistUserList" in rule_parameters:
        whitelist = rule_parameters['WhitelistUserList'].split(',')
        for user in whitelist:
            user_id = user.strip()
            if not user_id.isupper():
                raise ValueError("UserId's cannot have lowercase letters: " + user_id)
            elif not user_id.isalnum():
                raise ValueError("UserId's must be alphanumeric: " + user_id)
            elif user_id[0:4] != 'AIDA':
                error_string = "UserId's must start with 'AIDA': "
                error_string += user_id
                error_string += ".  Have you confused User ID's with User ARN's?"
                raise ValueError(error_string)
            elif user_id == 'AIDA':
                raise ValueError("UserId in whitelist is malformed: " + user_id)
            valid_rule_parameters.append(user_id)
